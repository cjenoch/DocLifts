...
- A server/data test suite that's real, not decoration. Vitest exercises the unit and database-integration paths against a real Postgres — the concurrency test above, plus snapshot semantics and history filtering. On every push, CI runs that suite against Postgres; a separate Browser CI workflow (push + nightly) runs the component harness. (Component/E2E coverage is currently thin — real UI-flow tests are the next testing step.)
